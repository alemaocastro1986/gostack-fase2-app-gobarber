...
describe('UpdateProfile', () => {
  ...
  it('Should be able to list the month availability from provider', async () => {
    ...
    expect(availability).toContainEqual({ hour: 12, available: false });
    expect(availability).toContainEqual({ hour: 13, available: true });
    expect(availability).toContainEqual({ hour: 14, available: true });
  });
});
